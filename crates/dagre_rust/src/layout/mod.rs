...
pub fn assign_node_intersects(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>) {
  for e in g.edges() {
    ...
    let (p1, p2) = if edge.points.is_none() || edge.points.as_ref().map_or(true, |p| p.is_empty()) {
...
pub fn remove_border_nodes(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>) {
  for v in g.nodes() {
    if g.children(&v).len() > 0 {
      ...
      let Some(border_top) = node.border_top.as_ref() else { continue; };
      let Some(border_bottom) = node.border_bottom.as_ref() else { continue; };
      let Some(t) = g.node(border_top).cloned() else { continue; };
      let Some(b) = g.node(border_bottom).cloned() else { continue; };
      let Some(border_left) = node.border_left.clone() else { continue; };
      let Some(border_right) = node.border_right.clone() else { continue; };
      let mut l_keys: Vec<i32> = border_left.keys().cloned().collect();
      if l_keys.is_empty() { continue; }
      l_keys.sort();
      let mut r_keys: Vec<i32> = border_right.keys().cloned().collect();
      if r_keys.is_empty() { continue; }
      r_keys.sort();
      let Some(l_node_id) = border_left.get(&l_keys[l_keys.len() - 1]) else { continue; };
      let Some(r_node_id) = border_right.get(&r_keys[r_keys.len() - 1]) else { continue; };
      let Some(l) = g.node(l_node_id).cloned() else { continue; };
      let Some(r) = g.node(r_node_id).cloned() else { continue; };
