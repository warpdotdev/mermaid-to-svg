...
use crate::layout::{GraphConfig, GraphEdge, GraphNode, util};
...
fn tree_depths(graph: &Graph<GraphConfig, GraphNode, GraphEdge>) -> OrderedHashMap<String, usize> {
  ...
  fn dfs(node_id: String, depth: usize, depths: &mut OrderedHashMap<String, usize>, graph: &Graph<GraphConfig, GraphNode, GraphEdge>) {
...
fn dfs(graph: &mut Graph<GraphConfig, GraphNode, GraphEdge>, root: &String, node_sep: &f32, weight: &f32, height: &usize, depths: &OrderedHashMap<String, usize>, node_id: &String) {
  let children = graph.children(node_id);
  if children.len() == 0 {
    if node_id != root {
  ...
  if graph.parent(node_id).is_none() {
    let mut graph_edge = GraphEdge::default();
    graph_edge.minlen = Some((depths.get(node_id).cloned().unwrap_or(0) + height.clone()) as f32);
    graph_edge.weight = Some(0.0);
