...
pub fn build_layer_graph(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, rank: &i32, relationship: GraphRelationship) -> Graph<GraphConfig, GraphNode, GraphEdge> {
  ...
  g.nodes().iter().for_each(|v| {
    ...
    if in_rank || in_subgraph_rank {
      ...
      _relationship.iter().for_each(|e| {
        ...
        edge_label.weight = Some(g.edge_with_obj(&e).unwrap().weight.clone().unwrap_or(0.0) + weight);
