...
pub fn sort(entries: &Vec<ResolvedBaryEntry>, bias_right: &bool) -> SubgraphResult {
  let parts: PartitionResponse<ResolvedBaryEntry> =
    util::partition(entries, Box::new(|val: &ResolvedBaryEntry| -> bool {
      val.barycenter.is_some()
    }));
...
fn compare_with_bias(entry_v: &ResolvedBaryEntry, entry_w: &ResolvedBaryEntry, bias: &bool) -> Ordering {
