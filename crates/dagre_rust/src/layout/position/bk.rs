 ...
 */
fn find_type_1_conflicts(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, layering: &Vec<Vec<String>>) -> OrderedHashMap<String, OrderedHashMap<String, bool>> {
  let mut conflicts = OrderedHashMap::new();

  fn visit_layer(
    g: &mut Graph<GraphConfig, GraphNode, GraphEdge>,
    prev_layer: &Vec<String>, layer: &Vec<String>,
      ...
      if w.is_some() || *v == last_node {
        for scan_node in layer[scan_pos..=i].iter() {
          for u in g.predecessors(scan_node).unwrap() {
            ...
            if (u_pos < k0 || k1 < u_pos) && !(u_label.dummy.is_some() && g.node(scan_node).unwrap().dummy.is_some()) {
      ...
      if g.node(&v).is_some() && g.node(&v).unwrap().dummy.is_some() {
        ...
        preds.iter().for_each(|u| {
          ...
          if let Some(u_node) = u_node_ {
            ...
            if u_node.dummy.is_some() && (u_node_order < prev_north_border || u_node_order > next_north_border) {
      ...
      if let Some(v_node) = g.node(v) {
        if v_node.dummy.as_deref() == Some("border") {
          ...
          if !predecessors.is_empty() {
            next_north_pos = g.node(&predecessors[0]).unwrap().order.unwrap_or(0) as i32;
            scan(
              g,
              south,
              south_pos,
...
fn find_other_inner_segment_node(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>, v: &String) -> Option<String> {
  if g.node(v).unwrap().dummy.is_some() {
    let preds = g.predecessors(v).unwrap_or(vec![]);
    return preds.iter().find(|u| {
      g.node(u).unwrap().dummy.is_some()
    }).cloned();
  }
...
pub fn add_conflict(conflicts: &mut OrderedHashMap<String, OrderedHashMap<String, bool>>, v_: &String, w_: &String) {
  let mut v = v_.clone();
  let mut w = w_.clone();
...
pub fn has_conflict(conflicts: &OrderedHashMap<String, OrderedHashMap<String, bool>>, v_: &String, w_: &String) -> bool {
  let mut v = v_;
  let mut w = w_;
  ...
  layering.iter().for_each(|layer| {
    ...
    layer.iter().for_each(|v| {
      ...
      if ws.len() > 0 {
        ...
        while i <= il {
          let w = ws[i].clone();
          if align.get(v).unwrap() == v && prev_idx < (pos.get(&w).cloned().unwrap() as i32) && !has_conflict(conflicts, v , &w) {
...
pub fn horizontal_compaction(
  ...
  let block_g: Graph<GraphOption, String, f32> = build_block_graph(g, layering, root, reverse_sep);
  ...
  fn iterate(
    set_xs_func: fn(&String, &mut OrderedHashMap<String, f32>,&Graph<GraphOption, String, f32>, &Graph<GraphConfig, GraphNode, GraphEdge>, &BorderTypeName),
  ...
  fn pass2(
    ...
    if min != f64::INFINITY {
      ...
      if node.border_type.as_ref() != Some(border_type) {
        xs.insert(elem.clone(), xs.get(elem).cloned().unwrap_or(0.0).max(min as f32));
      }
    }
  }

  iterate(pass1, Box::new(|block_g, v| -> Vec<String> {
    block_g.predecessors(v).unwrap_or(vec![])
  }), &block_g, &mut xs, g, &border_type);

  iterate(pass2, Box::new(|block_g, v| -> Vec<String> {
    block_g.successors(v).unwrap_or(vec![])
  }), &block_g, &mut xs, g, &border_type);
...
pub fn build_block_graph(g: &Graph<GraphConfig, GraphNode, GraphEdge>, layering: &Vec<Vec<String>>, root: &OrderedHashMap<String, String>, reverse_sep: bool) -> Graph<GraphOption, String, f32> {
  let mut block_graph: Graph<GraphOption, String, f32> = Graph::new(None);
  let graph_label = g.graph();
  let sep_fn: Box<dyn Fn(&Graph<GraphConfig, GraphNode, GraphEdge>, &String, &String) -> f32> = sep(
  ...
  layering.iter().for_each(|layer| {
    ...
    layer.iter().for_each(|v| {
      ...
      if u.is_some() {
        ...
        if u_root != v_root {
          let prev_max = block_graph.edge(&u_root, &v_root, None).cloned().unwrap_or(0.0);
 ...
 */
fn align_coordinates(xss: &mut OrderedHashMap<String, OrderedHashMap<String, f32>>, align_to: &OrderedHashMap<String, f32>) {
  let align_to_vals: Vec<f32> = align_to.values().cloned().collect();
  let align_to_min = align_to_vals.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap().clone();
  let align_to_max = align_to_vals.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap().clone();
  ...
  vec!["u", "d"].iter().for_each(|vert| {
    vec!["l", "r"].iter().for_each(|horiz| {
      ...
      let delta = if *horiz == "l" {
        align_to_min - *xs_vals.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()
      } else {
        align_to_max - *xs_vals.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()
      };
...
pub fn balance(xss: &OrderedHashMap<String, OrderedHashMap<String, f32>>, align: Option<String>) -> OrderedHashMap<String, f32> {
    ...
    keys.iter().for_each(|v| {
      if align.is_some() {
        ...
        let _balance = xss
          ...
          .get(v).cloned().unwrap_or(0.0);
        let item = ul.get_mut(v).unwrap();
        *item = _balance;
      } else {
        let mut xs: Vec<f32> = xss.values()
          .map(|_xs| _xs.get(v).cloned().unwrap_or(f64::INFINITY as f32)).collect();
...
pub fn position_x(g: &mut Graph<GraphConfig, GraphNode, GraphEdge>) -> OrderedHashMap<String, f32> {
  ...
  vec!["u", "d"].iter().for_each(|vert| {
    ...
    vec!["l", "r"].iter().for_each(|horiz| {
      if horiz == &"r" {
        adjusted_layering.as_mut().unwrap().iter_mut().for_each(|inner| {
          inner.reverse()
        });
      }

      let neighbor_fn: Box<dyn Fn(&Graph<GraphConfig, GraphNode, GraphEdge>, &String) -> Vec<String>> = if vert == &"u" {
        Box::new(|g: &Graph<GraphConfig, GraphNode, GraphEdge>, v: &String| -> Vec<String> {
      ...
      let align = vertical_alignment(g, adjusted_layering.as_ref().unwrap(), &conflicts, neighbor_fn);
      let mut xs = horizontal_compaction(g, adjusted_layering.as_ref().unwrap(), &align.0, &align.1, horiz == &"r");
      if horiz == &"r" {
...
fn sep(node_sep: &f32, edge_sep: &f32, reverse_sep: &bool) -> Box<dyn Fn(&Graph<GraphConfig, GraphNode, GraphEdge>, &String, &String) -> f32> {
  let node_sep_ = node_sep.clone();
  let edge_sep_ = edge_sep.clone();
  let reverse_sep_ = reverse_sep.clone();
  Box::new(move |g: &Graph<GraphConfig, GraphNode, GraphEdge>, v: &String, w: &String| -> f32 {
