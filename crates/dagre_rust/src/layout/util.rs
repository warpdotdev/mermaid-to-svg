...
pub fn remove_empty_ranks(graph: &mut Graph<GraphConfig, GraphNode, GraphEdge>) {
  ...
  let node_ranks: Vec<i32> = nodes
    .iter()
    .map(|v| graph.node(v).cloned().unwrap_or(GraphNode::default()).rank.unwrap_or(0))
    .collect();
  ...
  for v in &nodes {
    let rank = graph.node(v).unwrap_or(&GraphNode::default()).rank.unwrap_or(0) - offset;
    if rank >= 0 {
...
pub fn max_rank(g: &Graph<GraphConfig, GraphNode, GraphEdge>) -> i32 {
  g.nodes().iter().filter_map(|v| {
    g.node(v).and_then(|n| n.rank)
  }).max().unwrap_or(0)
}
