...
fn render_entity_node(svg: &mut String, entity: &EntityLayout, theme: &MermaidTheme) {
    ...
    if !entity.attributes.is_empty() {
        ...
        // In the default theme: rowOdd = lighten(primary, 75) ≈ #ffffff,
        // rowEven = lighten(primary, 1) ≈ slightly lighter than node_fill.
        let row_odd_fill = String::from("#ffffff");
        let row_even_fill = lighten_hex(&theme.node_fill, 0.25);
